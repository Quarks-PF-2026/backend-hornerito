...
| **DoD** | Al cerrar la tarea | Cinco condiciones: implementada, integrada (suite completa en verde, sin regresión), probada (gate GREEN), documentada (ADR si hubo decisión arquitectónica, `DOMAIN.md` si cambió una regla) y validada por el Product Owner. Las primeras cuatro las verifica el ciclo; la quinta es externa — Liliana Costabello valida en la Sprint Review. El ciclo transiciona el ticket en Jira a `En revisión` y deja explícito que la transición a `Finalizado` queda pendiente de esa quinta condición, y es humana (detalle de estados en la skill `jira-ticket`) |
